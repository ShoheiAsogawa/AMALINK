...
function NewsItem({ item }: { item: News }) {
  return (
    ...
    >
      ...
      <p className="text-sm md:text-base text-slate-700 font-sans truncate group-hover:text-amami-blue transition-colors duration-300">
        {item.title}
      </p>

      <ArrowRight className="w-4 h-4 text-slate-300 group-hover:text-amami-blue group-hover:translate-x-1 transition-all duration-300 hidden md:block" />
    </Link>
  );
}
...
export function NewsSection({ news }: { news: News[] }) {
  return (
    <section
      id="news"
      className="bg-gradient-to-b from-white to-slate-50 py-20 md:py-32 relative overflow-hidden"
    >
      <WaveBackground
        color="green"
        position="bottom"
